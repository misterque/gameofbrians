...
#define WIDTH 5   	// width of each cell 
#define HEIGHT 5  	// height of each cell
 ...
 rect.h = WIDTH;
  ...
  for (int y = 0;y<100;y++)
    for (int x = 0;x<100;x++)
    {
      rect.x = x*WIDTH;
      rect.y = y*HEIGHT;
      color = SDL_MapRGB(screen->format, Feld[x][y].r,
 	                                 Feld[x][y].g,
		                         Feld[x][y].b);
     if (Feld[x][y].alive == true) SDL_FillRect(screen, &rect, color);
     if (Feld[x][y].alive == false) SDL_FillRect(screen, &rect, black);
    };    
...
void CPCell(Cell* Zelle){
  if(dodeathfunc && Zelle->resswert < 5	) return;
...
int main (int argc, char** argv)
{
  ...
  screen = SDL_SetVideoMode(SIZEX*WIDTH, SIZEY*HEIGHT, 16, SDL_SWSURFACE);
  int showper = SHOWPER; 
  while (!done)
  {
    ...
    while(SDL_PollEvent(&event))
      ...
      {
        case SDL_KEYDOWN:
        if (event.key.keysym.sym==SDLK_F1) init();		// reset
        if (event.key.keysym.sym==SDLK_F2) dodeathfunc=!dodeathfunc;
        if (event.key.keysym.sym==SDLK_F4) showper = 0;		// pause
        if (event.key.keysym.sym==SDLK_F5) showper = 1;		// speed mod
        if (event.key.keysym.sym==SDLK_F6) showper = 100;        //
        if (event.key.keysym.sym==SDLK_F7) showper = 1000;       //
        if (event.key.keysym.sym==SDLK_F8) showper = 10000;      //
        if (event.key.keysym.sym==SDLK_F9)  mutarate = 1000;		// speed mod
        if (event.key.keysym.sym==SDLK_F10) mutarate = 100;        //
        if (event.key.keysym.sym==SDLK_F11) mutarate = 10;       //
        if (event.key.keysym.sym==SDLK_F12) mutarate = 1;      //
